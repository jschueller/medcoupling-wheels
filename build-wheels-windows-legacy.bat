...
REM curl -LO https://files.salome-platform.org/Salome/other/med-%MED_VERSION%.tar.gz
...
mkdir .\build_dir\medcoupling
xcopy /y C:\Libraries\medcoupling\lib\python%PY_VER%\site-packages .\build_dir\medcoupling
...
REM pushd C:\Libraries\medcoupling\lib\python%PY_VER%\site-packages
REM mkdir medcoupling-%VERSION%.dist-info
REM sed "s|@PACKAGE_VERSION@|%VERSION%|g" %GITHUB_WORKSPACE%\METADATA.in > medcoupling-%VERSION%.dist-info\METADATA
REM type medcoupling-%VERSION%.dist-info\METADATA
REM echo Wheel-Version: 1.0 > medcoupling-%VERSION%.dist-info\WHEEL
REM echo medcoupling-%VERSION%.dist-info\RECORD,, > medcoupling-%VERSION%.dist-info\RECORD
REM mkdir %GITHUB_WORKSPACE%\wheelhouse
REM 7z a -tzip %GITHUB_WORKSPACE%\wheelhouse\medcoupling-%VERSION%-%ABI%-%ABI%-win_amd64.whl *.py *.pyd *.dll medcoupling-%VERSION%.dist-info
REM pip install %GITHUB_WORKSPACE%\wheelhouse\medcoupling-%VERSION%-%ABI%-%ABI%-win_amd64.whl
REM pushd %GITHUB_WORKSPACE%
REM
